...
object UrlParseUtils {
    ...
    fun lanzou(url: String, pwd: String = "") = Gson().fromJson<LanouApi>(OkHttpUtils.get("https://lanzou.uyclouds.com/v1/parseByUrl?type=old&url=$url&pwd=$pwd").excuteString(), LanouApi::class.java).url
    ...
    fun ilanzou(url: String, pwd: String = ""): String{
        ...
        "<iframe.*?name=\"[\\s\\S]*?\"\\ssrc=\"/(.*?)\"".toRegex().find(softInfo!!)?.groupValues?.get(1)?.let { link ->
            val signInfo = OkHttpUtils.get(host+link).header("User-Agent", userAgent).excuteString()
            "url\\s*:\\s*'/(.*?)'[\\s\\S]*'sign':'(.*?)'".toRegex().find(signInfo!!)?.groupValues?.let {
                println(it)
                val downInfo =  Gson().fromJson(
                    OkHttpUtils.post(host+it[1]).formDataBody(mapOf(
